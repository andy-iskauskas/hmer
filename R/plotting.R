...
imp_plot <- function(em, z, plotgrid = NULL, ppd = 30, cb = FALSE, nth = NULL, imp_breaks = NULL) {
  if (!is.null(nth)) {
    if (nth == 1) ns <- ""
...
#' @param imp_breaks An increasing numeric vector of break points for implausibility
#'                   colour scales. Useful if custom implausibility functions are
#'                   being used.
#'
