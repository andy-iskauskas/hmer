HierarchicalEmulator <- R6Class(
  ...
  public = list(
    ...
    initialize = function(basis_f, beta, u, ranges, data = NULL, model = NULL, original_em = NULL, out_name = NULL, a_vars = NULL, discs = NULL, s_diag = NULL, samples = 0, multiplier = 1) {
      ...
      if (!is.null(self$in_data)) {
        ...
        if (is.numeric(self$u_sigma))
          d_corr <- self$u_sigma^2 * self$corr$get_corr(self$in_data, actives = self$active_vars) + diag(sample_mod, nrow = nrow(self$in_data))
        else
          d_corr <- diag(apply(self$in_data, 1, self$u_sigma)^2, nrow = nrow(self$in_data)) %*% self$corr$get_corr(self$in_data, actives = self$active_vars) + diag(sample_mod, nrow = nrow(self$in_data))
    ...
    implausibility = function(x, z, cutoff = NULL) {
      ...
      if (all(self$disc == 0)) {
        if (!is.numeric(z) && !is.null(z$val)) self$disc$external <- 0.05 * z$val
    ...
    adjust = function(data, out_name) {
      ...
      if (all(eigen(self$beta_sigma)$values == 0)) {
        ...
        sample_mod <- purrr::map_dbl(seq_along(1:nrow(temp_in)), ~self$s_diag(temp_in[.,], self$samples[.]))
        Ot <- self$corr$get_corr(this_data_in, actives = self$active_vars) + diag(sample_mod, nrow = nrow(this_data_in))
        O <- tryCatch(
          chol2inv(chol(Ot)),
          error = function(x) {
            MASS::ginv(Ot)
      ...
      new_em <- HierarchicalEmulator$new(self$basis_f, beta = list(mu = new_beta_exp, sigma = new_beta_var),
