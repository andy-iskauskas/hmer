...
```{r}
emulator_from_data(data1d, c('f'), ranges1d)$f
em1d$f$o_em
```
...
plot1d <- data.frame(
  ...
  f = func(test1d$x),
  E = em1d_exp,
  max = em1d_exp + 3*sqrt(em1d_var),
  min = em1d_exp - 3*sqrt(em1d_var)
)
...
```{r}
new_points1d <- generate_new_runs(em1d, 10, target1d, method = 'lhs', measure.method = 'maximin')
```
...
plot1d_2 <- data.frame(
  ...
  min = em1d_2$f$get_exp(test1d) - 3*sqrt(em1d_2$f$get_cov(test1d)),
  max = em1d_2$f$get_exp(test1d) + 3*sqrt(em1d_2$f$get_cov(test1d))
)
...
```{r, fig.width = 7, fig.height = 7}
new_new_points1d <- generate_new_runs(c(em1d_2, em1d), 10, z = c(target1d, target1d), method = 'lhs', measure.method = 'maximin')
...
```{r fig.width = 7, fig.height = 7}
invalid_points <- validation_diagnostics(ems2d, validation_data, targets2d)
```
...
```{r fig.width = 7, fig.height = 7}
inflated_em <- ems2d$f2$set_sigma(ems2d$f2$u_sigma*2)
new_invaliid <- validation_diagnostics(list(f1 = ems2d$f1, f2 = inflated_em), validation_data, targets2d)
```
...
```{r fig.width = 7, fig.height = 7}
emulator_plot(ems2d$f1, var_name = 'sd') + geom_point(data = initial_data, aes(x = x, y = y))
```
...
```{r fig.width = 7, fig.height = 7}
emulator_plot(ems2d, var_name = 'imp', targets = targets2d)
emulator_plot(ems2d, var_name = 'nimp', targets = targets2d)
```
...
```{r fig.width = 7, fig.height = 7}
invalid_points2 <- validation_diagnostics(ems2d_2, valid2d, targets2d)
```
