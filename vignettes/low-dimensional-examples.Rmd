...
The structure pf this plot warrants some discussion. In the one-dimensional case, we discussed the fact that the emulator was 'certain' at training points, and less so elsewhere; the uncertainty of the emulator was driven by its distance from a known training point. Here we have the same picture: minima of the emulator uncertainty (coloured here in deep blue) sit directly over training points. As we move away from a training point, the uncertainty increases until at the edges (which represent the maximum distance from training points) it reaches a maximum. The gradient of this change in uncertainty is controlled by the correlation length - a higher correlation length presumes that nearby points are more closely correlated, and in some sense can be equated to the 'smoothness' of our output space. In any event, we may look at the difference between correlation lengths by looking at the two emulator uncertainty plots.

```{r fig.width = 7, fig.height = 7}
ems2d$f1$corr$hyper_p$theta
ems2d$f2$corr$hyper_p$theta
emulator_plot(ems2d$f2, 'sd') + geom_point(data = initial_data, aes(x = x, y = y))
```
...
```{r fig.width = 7, fig.height = 7}
invalid_points2 <- validation_diagnostics(ems2d_2, targets2d, valid2d, plt = TRUE)
```
...
```{r fig.width = 7, fig.height = 7}
new_new_points2d <- generate_new_runs(c(ems2d_2, ems2d), 40, c(targets2d, targets2d), resample = 0)
