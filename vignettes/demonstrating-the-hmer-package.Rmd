...
We see, for instance, that choosing a cutoff of 3 (roughly corresponding to a 3-sigma deviation from the target value) is sufficient to rule out over 95% of the space - coupled with the fact that our diagnostics were so healthy suggests that this is a very reasonable first cut.
...
simulator_plot(SIRMultiWaveData, targets)
```
...
```{r, fig.height = 7, fig.width = 7}
diagnostic_wrap(SIRMultiWaveData, targets, include.norm = FALSE, include.log = FALSE)
```

Given an equivalent list of emulators (here using `SIRMultiWaveEmulators`), we can see the evolution of the emulator variance across the waves. Note that only three rows, each corresponding to a wave, are produced, since the first element of the MultiWaveData contains wave '0' points, to which no emulators are associated.

```{r, fig.height = 7, fig.width = 7}
wave_variance(SIRMultiWaveEmulators, names(targets))
```
...
```{r}
f_w <- suppressWarnings(full_wave(SIRSample$training, list(aSI = c(0.1, 0.8), aIR = c(0, 0.5), aSR = c(0, 0.05)), targets))
```
