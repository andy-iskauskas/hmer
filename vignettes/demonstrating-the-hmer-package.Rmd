...
invalid_points <- validation_diagnostics(wave1_emulators, targets, GillespieValidation, plt = TRUE)
```
...
```{r fig.width = 7, fig.height = 7}
space_removed(wave1_emulators, targets, ppd = 10) + geom_vline(xintercept = 3)
```

We see, for instance, that choosing a cutoff of 3 (roughly corresponding to a 3-sigma deviation from the target value) is sufficient to rule out over 80% of the space, and that increasing or decreasing observational uncertainly will not greatly change this outcome.
...
```{r}
f_w <- suppressWarnings(full_wave(GillespieSIR, list(aSI = c(0.1, 0.8), aIR = c(0, 0.5), aSR = c(0, 0.05)), targets))
```
