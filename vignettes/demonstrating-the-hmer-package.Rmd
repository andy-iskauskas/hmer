...
One could instead specify a different correlation function, for instance Matern:
...
For these plots, we will use the `SIRMultiWaveData` dataset provided. This contains a list of four waves of points (including wave 0), along with the model evaluation. We can use this to look at dependencies of the outputs on the inputs (`wave_dependencies`), the evolution of the output values obtained (`wave_values`), and the overall behaviour of the model runs (`simulator_plot`).
