...
We use a set of (included) model run datasets to demonstrate the functionality - all have been generated from an SIRS model solved using the Gillespie algorithm. By and large in the document, we will not focus on emulating the variance of this stochastic system.

## Training emulators
The first step, given a dataset (here the `GillespieSIR` set), is to train an emulator to each output. We need to provide details about the parameters (and their ranges) to the model and the outputs from the model, as well as the data itself. Then the function `emulator_from_data(data, outputs, ranges, ...)` produces a set of `Emulator` objects, as a named list.

```{r}
input_ranges <- list(aSI = c(0.1, 0.8), aIR = c(0, 0.5), aSR = c(0, 0.05))
output_names <- c('nS', 'nI', 'nR')
wave1_emulators <- emulator_from_data(GillespieSIR, output_names, input_ranges)
```
...
invalid_points <- validation_diagnostics(wave1_emulators, targets, GillespieValidation)
```
...
emulator_plot(wave1_emulators$nS, params = c('aIR', 'aSR'), fixed_vals = list(aSI = 0.3))
emulator_plot(wave1_emulators$nI, var_name = 'var')
emulator_plot(wave1_emulators$nI, var_name = 'sd')
```

Almost all the return values of plots produced by `hmer` are `ggplot` objects, and so can be augmented after the fact.

```{r fig.width = 7, fig.height = 7}
emulator_plot(wave1_emulators$nS, params = c('aIR', 'aSR')) + ggplot2::geom_point(data = GillespieSIR, ggplot2::aes(x = aIR, y = aSR))
```
...
``` {r fig.width = 7, fig.height = 7}
emulator_plot(wave1_emulators, var = 'imp', targets = targets)
emulator_plot(wave1_emulators, var_name = 'nimp', targets = targets, cb = TRUE)
```

While these plots represent slices of parameter space, other functionality allows a rough understanding of the behaviour across the full space.In particular, the `plot_lattice` function provides a plot of optical depth and minimum implausibility for all parameter combinations.
...
```{r fig.width = 7, fig.height = 7}
space_removed(wave1_emulators, targets, ppd = 10) + ggplot2::geom_vline(xintercept = 3)
```

We see, for instance, that choosing a cutoff of 3 (roughly corresponding to a 3-sigma deviation from the target value) is sufficient to rule out over 60% of the space, and that increasing or decreasing observational uncertainly will not greatly change this outcome.

Eventually, the aim of history matching is to propose new points which can enter into the model, the results of which can train new emulators, repeating the cycle. The `generate_new_runs` function does just that. There are multiple options that can be passed to this function: the default behaviour should be adequate for most situations, with the consideration of the cutoff value mentioned above. Information about the progress of the point proposal can be obtained by setting the argument `verbose = TRUE`.

```{r fig.height = 7, fig.width = 7}
new_points <- generate_new_runs(wave1_emulators, 90, targets)
plot(rbind(rbind(GillespieSIR, GillespieValidation)[,names(input_ranges)], new_points), pch = 16, cex = 0.8, col = rep(c('black', 'blue'), each = 90))
```
...
```{r}
wave.points <- list(GillespieSIR[,c('aSI', 'aIR', 'aSR')], new_points)
```
...
For these plots, we will use the `GillespieMultiWaveData` dataset provided. This contains a list of four waves of points (including wave 0), along with the model evaluation. We can use this to look at dependencies of the outputs on the inputs (`wave_dependencies`), the evolution of the output values obtained (`wave_values`), and the overall behaviour of the model runs (`simulator_plot`).

```{r, fig.height = 7, fig.width = 7}
wave_values(GillespieMultiWaveData, targets)
wave_dependencies(GillespieMultiWaveData, targets)
simulator_plot(GillespieMultiWaveData, targets)
```

All of these plots, as well as `wave_points`, are wrapped into a single function, `diagnostic_wrap`. Normalisation options are available for target values (log-scaled or normalising all targets to the range $[-1,1]$), but we suppress those plots here using `include.norm = FALSE` and `include.log = FALSE`.

```{r, fig.height = 7, fig.width = 7}
diagnostic_wrap(GillespieMultiWaveData, targets, include.norm = FALSE, include.log = FALSE)
```

Given an equivalent list of emulators (here using `GillespieMultiWaveEmulators`), we can see the evolution of the emulator variance across the waves. Note that only three rows, each corresponding to a wave, are produced, since the first element of the MultiWaveData contains wave '0' points, to which no emulators are associated.

```{r, fig.height = 7, fig.width = 7}
wave_variance(GillespieMultiWaveEmulators, names(targets))
```
...
```{r}
f_w <- full_wave(GillespieSIR, list(aSI = c(0.1, 0.8), aIR = c(0, 0.5), aSR = c(0, 0.05)), targets)
```

This provides two things: a next set of points to evaluate the model on, and the emulators that gave rise to them.
